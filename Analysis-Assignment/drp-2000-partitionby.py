...
newdf = df2.select('Weather_Station', 'WBAN','Observation_Date','Observation_Hour','Latitude','Longitude','Elevation','Wind_Direction','WD_Quality_Code','Sky_Ceiling_Height','SC_Quality_Code','Visibility_Distance','VD_Quality_Code','Air_Temperature','AT_Quality_Code', 'Dew_Point','DP_Quality_Code','Atmospheric_Pressure','AP_Quality_Code', month(df2.Observation_Date).alias('month'))

#Chapter 09 pg 183 of E-book Partitioning

newdf.write.partitionBy('month').format("parquet").mode("overwrite").option("header","true").save("hdfs://namenode/output/itmd-521/drp/2000/Partition-By-Month-Parquet")
